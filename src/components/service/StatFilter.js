...
const StatFilter = () => {
  ...
  return (
    <div className="button-container">
      ...
      {currentStat == "threes" && (
        <button
          disabled={orderBy === "percentage"}
          onClick={() => setOrderBy("percentage")}
        >
          Percentages
        </button>
      )}
      {currentStat == "fouls" && (
        <button disabled={orderBy === "average"}>Averages</button>
      )}
      {currentStat == "foul_outs" && (
        <button disabled={orderBy === "total"}>Totals</button>
      )}
      {currentStat == "technicals" && (
        <button disabled={orderBy === "total"}>Totals</button>
      )}
    </div>
  );
